...
import { UsuarioDetalheDialog } from '../../usuario/lista/detalhe';


@Component({
  selector: 'app-empresa-usuario-list',
...
export class ListComp implements OnInit, OnDestroy {
  ...
  @Input({ required: true }) empresa!: Empresa | null;
  ...
  buscarUsuarios() {
    if (this.empresa && this.empresa.id) { // Ensure cliente and cliente.id exist
      this.spinner
        .showUntilCompleted(this.usuarioService.buscarPorEmpresa(this.ctrlFiltro.value, this.empresa.id, this.page()))
        .subscribe({
          next: (result) => {
